...
public class ProgressWebView extends WebView {
    ...
    private void initSettings() {
        ...
        mSettings.setJavaScriptEnabled(false); // 关闭 JavaScript
        mSettings.setDomStorageEnabled(true);  // 开启 DOM
        ...
        mSettings.setUseWideViewPort(true);         // 调整到适合 WebView 大小
        mSettings.setLoadWithOverviewMode(false);   // 调整到适合 WebView 大小
        // 屏幕自适应网页,如果没有这个,在低分辨率的手机上显示可能会异常
        mSettings.setDefaultZoom(WebSettings.ZoomDensity.FAR);
        mSettings.setRenderPriority(WebSettings.RenderPriority.HIGH);
        // 提高网页加载速度,暂时阻塞图片加载,等网页加载完成后再进行加载图片
